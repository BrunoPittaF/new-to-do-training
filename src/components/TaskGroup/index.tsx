...
import { TaskWrapper, TaskFormGroup, TaskListWrapper } from './style';
...
export function TaskGroup() {
    ...
    function onCompletedTask(id: number) {
        const newListTasks = Tasks.map((task: ITasks) =>
            task.id === id ? {
                ...task,
        ...
        const newCompletedListTasks = newListTasks.filter((task: ITasks) =>
            task.isCompleted === true);

        setcompletedTasks(newCompletedListTasks.length);
    }
    ...
    function onDeletedTask(id: number) {
        const newListTasks = Tasks.filter((task: ITasks) => (task.id !== id));
    ...
    return (
        <TaskWrapper>
            <TaskFormGroup>
                <input type='text' value={inputTask} onChange={(e) => setInputTask(e.target.value)} placeholder='Adicione uma nova tarefa'></input>
                <button onClick={createNewTask}>
                    Criar
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clipPath="url(#clip0_1501_11)">
            ...
            <TaskListWrapper>
                <header>
                    ...
                    <p>
                        Concluídas
                        <span>
                            {completedTasks} {Tasks.length > 0 ? `de ${Tasks.length}` : ''}
...

                {
                    Tasks.length == 0
                        ...
                        : Tasks.map((assignment) => {
                            return <Task key={assignment.id} onDeletedTask={() => onDeletedTask(assignment.id)} onCompletedTask={() => onCompletedTask(assignment.id)} content={assignment.content} completedTask={assignment.isCompleted} />;
                        })
                }
